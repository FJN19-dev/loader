...
local API_URL = "https://my-vercel-webhook.vercel.app/api/sendFruit"
...
-- Envia para a API no Vercel
local function SendToAPI(fruitName)
    ...
    while task.wait(0.5) do
        if getgenv().AutoFarm then
            pcall(function()
                ...
                -- ===============================
                -- 🚢 TRAVEL SUBMERGED ISLAND
                -- ===============================
                if player.Data.Level.Value >= 2675 and humanoidRoot.Position.Y > -500 then
                    local dist = (humanoidRoot.Position - NPC_Sub_Pos.Position).Magnitude
                    ...
                    return -- impede continuar a lógica de quest
                end
                ...
                if questVisible and not string.find(questTitle, NameMon) then
    ...
    while task.wait(0.5) do
        if getgenv().StopChest then
            ...
            if player then
                ...
                -- Só executa se backpack e character existirem
...
task.spawn(function()
    while true do
        ...
        if getgenv().AutoStoreFruits then
            pcall(function()
...
Fruit:AddButton({"Abrir Fruit Shop", function()
    local player = game:GetService("Players").LocalPlayer

    local FruitShopModule = require(
        player.PlayerGui.Main.UIController.FruitShop
    )



    pcall(function()
        FruitShopModule.Open()
    end)
end})
...
task.spawn(function()
    while task.wait(0.5) do
        if _G.StartRaid then  -- inverter a lógica
            pcall(function()
                local player = game.Players.LocalPlayer
...
-----Sea Event-----
local Section = Sea:AddSection({"Mar"})



local Section = Sea:AddSection({"Leviathan"})
local Toggle1 = Sea:AddToggle({
  Name = "Teleporta Até Frozen Dimension",
  Description = "",
  Default = false 
})
Toggle1:Callback(function(Value)
...
Sea:AddButton({"Destruir Lava da Ilha Pré-Histórica", function()
...
local Toggle1 = Sea:AddToggle({
  Name = "Teletransportar para a Ilha Pré-Histórica",
  Description = "",
  Default = false 
})
Toggle1:Callback(function(Value)
...
local Shop = L_1_[93].Shop
local CommF = L_1_[55].Remotes.CommF_

-- Funções auxiliares
local function BuyItem(item)
    CommF:InvokeServer("BuyItem", item)
end

local function BuyEctoplasm(id)
    CommF:InvokeServer("Ectoplasm", "Buy", id)
end
...
-- SECTION: Weapon World 1
Shop:AddSection({"Weapon World 1"})

Shop:AddButton({ Name = "Buy Cutlass", Description = "", Callback = function() BuyItem("Cutlass") end })
Shop:AddButton({ Name = "Buy Katana", Description = "", Callback = function() BuyItem("Katana") end })
Shop:AddButton({ Name = "Buy Iron Mace", Description = "", Callback = function() BuyItem("Iron Mace") end })
Shop:AddButton({ Name = "Buy Duel Katana", Description = "", Callback = function() BuyItem("Duel Katana") end })
Shop:AddButton({ Name = "Buy Triple Katana", Description = "", Callback = function() BuyItem("Triple Katana") end })
Shop:AddButton({ Name = "Buy Pipe", Description = "", Callback = function() BuyItem("Pipe") end })
Shop:AddButton({ Name = "Buy Dual-Headed Blade", Description = "", Callback = function() BuyItem("Dual-Headed Blade") end })
Shop:AddButton({ Name = "Buy Bisento", Description = "", Callback = function() BuyItem("Bisento") end })
Shop:AddButton({ Name = "Buy Soul Cane", Description = "", Callback = function() BuyItem("Soul Cane") end })
Shop:AddButton({ Name = "Buy Slingshot", Description = "", Callback = function() BuyItem("Slingshot") end })
Shop:AddButton({ Name = "Buy Musket", Description = "", Callback = function() BuyItem("Musket") end })
Shop:AddButton({ Name = "Buy Dual Flintlock", Description = "", Callback = function() BuyItem("Dual Flintlock") end })
Shop:AddButton({ Name = "Buy Flintlock", Description = "", Callback = function() BuyItem("Flintlock") end })
Shop:AddButton({ Name = "Buy Refined Flintlock", Description = "", Callback = function() BuyItem("Refined Flintlock") end })
Shop:AddButton({ Name = "Buy Cannon", Description = "", Callback = function() BuyItem("Cannon") end })
...
Shop:AddButton({
    ...
    function()
        ...
        repeat
            task.wait()
        until (hrp.Position - targetPosition).Magnitude < 1
        ...
        local remoteName = L_1_[2]({
            "RF/InteractDragonQue",
            "st"
        })

        Net:FindFirstChild(remoteName):InvokeServer(unpack(interactData))
    end
})
...
-- Define padrão como true
getgenv().FastAttack = true

-- Cria o toggle já ligado
local Toggle1 = Settings:AddToggle({
    Name = "Fast Attack",
    Description = "",
    Default = getgenv().FastAttack -- <- garante que o toggle começa ligado
})
...
getgenv().BringMonster = true -- Ativa por padrão
    ...
    while task.wait(0.1) do
        if getgenv().BringMonster then
            pcall(function()
                ...
                for _, enemy in ipairs(enemies) do
                    if MonsterCount >= 2 then break end
                    ...
                    if enemy:FindFirstChild("Humanoid")
                        ...
                        if getgenv().StartMagnet
                            ...
                            if enemy.Name == "Factory Staff" and (rootPart.Position - PosMon.Position).Magnitude <= 5000 then
                                canMove = true
