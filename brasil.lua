...
-- ===== Configurações =====
-- ========= CONFIGURAÇÃO =========
local WEBHOOK = "https://discord.com/api/webhooks/1361721393116614798/M_UbO9uxaMt874aMlOAhr4E4_yF6T8GNznuQGgZSpvf5JGNLAZQQ44yEsaBP6pkfuHgN"
...
local MirageWebhook = "https://discord.com/api/webhooks/1361721403384008935/Tbx1Bc8OR3FNEDxDQT8lp6147Zzt9Y3TLa0xq4gLW6j2m0B22FU2Sy1m5Tr1QqMRkOcT"
...
local MoonWebhook = "https://discord.com/api/webhooks/1361721400167239690/NTb_tTi-ySlBcenPwjPa-qcI5HTa12IqxFWihDLOBQvIGWtm9fdksY4KAPTNJvz1siXU"
...
local ToggleMobAura = Tabs.Main:AddToggle("ToggleMobAura", {Title = "Auto Farm Sky Piea 1-100", Default = false})
    ...
    while task.wait(0.03) do -- atualiza ~30 vezes por segundo
        if _G.AutoFarmSkyPiea then
            pcall(function()
                ...
                elseif MyLevel > 50 and MyLevel <= 100 then
                    Target = "Shanda"
...
-- Lista das ilhas e seus respectivos CFrames para cada Sea
...
-- Função para verificar qual Sea o jogador está com base no PlaceId
function getSea()
...
-- Função para criar o ESP nas ilhas
function createESPForIsland(name, CFramePos, offsetY)
    -- Criar um Part (parte invisível para a base do ESP)
    ...
    islandPart.Name = "IslandESP"  -- Dando um nome para que possamos identificar e remover depois

    -- Criar o BillboardGui (o ESP)
    local billboard = Instance.new("BillboardGui")
    billboard.Parent = islandPart
    billboard.Adornee = islandPart
    billboard.Size = UDim2.new(0, 200, 0, 50)
    billboard.StudsOffset = Vector3.new(0, offsetY, 0)  -- Elevação ajustada com base no offsetY
    ...
    -- Atualizar a distância entre o jogador e a ilha em tempo real
...
-- Função para criar o ESP com base no Sea do jogador
function createESPForCurrentSea()
    local sea = getSea()  -- Determina em qual Sea o jogador está
    local offsetY = 5  -- Começa com a altura padrão

    local islandsToUse = {}  -- Lista de ilhas a serem usadas
    if sea == 1 then
        islandsToUse = islandCFramesSea1
    elseif sea == 2 then
        islandsToUse = islandCFramesSea2
    elseif sea == 3 then
        islandsToUse = islandCFramesSea3
    end

    -- Criar o ESP para as ilhas do Sea atual
    for _, island in ipairs(islandsToUse) do
        createESPForIsland(island.name, island.position, offsetY)
        offsetY = offsetY + 2  -- Aumenta a altura do próximo ESP
    end
end

-- Função para remover os ESPs (Part e BillboardGui)
function removeESP()
    for _, islandPart in ipairs(workspace:GetChildren()) do
        if islandPart:IsA("Part") and islandPart.Name == "IslandESP" then
            islandPart:Destroy()  -- Remove o Part (ESP)
        end
    end
end

-- Criar o toggle para ativar/desativar o ESP
local Toggle = Tabs.Players:AddToggle("Esp Island", {Title = "Esp Island", Default = false})

Toggle:OnChanged(function(Value)
    if Value then
        createESPForCurrentSea()  -- Ativar o ESP quando o toggle for ligado
    else
        removeESP()  -- Remover os ESPs quando o toggle for desligado
    end
end)

-- Configurações
local player = game.Players.LocalPlayer -- Jogador local
local character = player.Character or player.CharacterAdded:Wait() -- Personagem do jogador

-- Função para criar o ESP (BillboardGui) para os baús
local function createChestESP(chest)
    if (chest.Name == "SilverChest" or chest.Name == "GoldChest" or chest.Name == "DiamondChest") and not chest:FindFirstChild("NameTag") then
        -- Criar BillboardGui
        local billboard = Instance.new("BillboardGui")
        billboard.Name = "NameTag"
        billboard.Parent = chest

        -- Verificar a PrimaryPart ou tentar encontrar um BasePart
        local adornee = chest.PrimaryPart or chest:FindFirstChildWhichIsA("BasePart")
        if not adornee then
            warn("Nenhuma PrimaryPart ou BasePart encontrada para: " .. chest.Name)
            return
        end

        billboard.Adornee = adornee
        billboard.Size = UDim2.new(0, 200, 0, 50)
        billboard.StudsOffset = Vector3.new(0, 3, 0) -- Altura acima do baú
        billboard.AlwaysOnTop = true
        ...
        -- Definir a cor do texto com base no tipo de baú
        if chest.Name == "SilverChest" then
            textLabel.TextColor3 = Color3.new(0.75, 0.75, 0.75) -- Cor prata para SilverChest
        elseif chest.Name == "GoldChest" then
            textLabel.TextColor3 = Color3.new(1, 0.84, 0) -- Cor dourada para GoldChest
        elseif chest.Name == "DiamondChest" then
            textLabel.TextColor3 = Color3.new(0.4, 0.8, 1) -- Cor azul-celeste para DiamondChest
        end

        textLabel.TextStrokeTransparency = 0 -- Contorno do texto
        textLabel.TextStrokeColor3 = Color3.new(0, 0, 0) -- Contorno preto para contraste
        textLabel.Font = Enum.Font.GothamBold
        textLabel.TextSize = 16
        textLabel.Text = chest.Name -- Nome do baú

        -- Atualizar o texto com a distância
        game:GetService("RunService").RenderStepped:Connect(function()
            if chest and adornee and character.PrimaryPart then
                local distance = (adornee.Position - character.PrimaryPart.Position).Magnitude
                local distanceInMeters = math.floor(distance * 10) / 10 -- Arredonda para 1 casa decimal
                textLabel.Text = string.format("%s\n(%.1fm)", chest.Name, distanceInMeters)
            end
        end)
    end
end

-- Função para procurar e criar o ESP para todos os baús (SilverChest, GoldChest e DiamondChest) na workspace
local function searchAndAddChestsESP()
...
-- Detectar baús SilverChest, GoldChest e DiamondChest automaticamente quando são adicionados
workspace.ChildAdded:Connect(function(child)
    if (child.Name == "SilverChest" or child.Name == "GoldChest" or child.Name == "DiamondChest") and child:IsA("Model") then
        createChestESP(child)
    end
end)

-- Adicionar ESP aos baús SilverChest, GoldChest e DiamondChest existentes no momento
searchAndAddChestsESP()

-- Toggle para controlar o ESP
local Toggle = Tabs.Players:AddToggle("ToggleESP", {Title = "Esp Chest", Default = false})

Toggle:OnChanged(function(state)
    -- Desativa ou ativa o ESP com base no toggle
    for _, chest in pairs(workspace:GetDescendants()) do
        if (chest.Name == "SilverChest" or chest.Name == "GoldChest" or chest.Name == "DiamondChest") and chest:IsA("Model") then
            if state then
                -- Se o toggle estiver ativado, cria o ESP
                createChestESP(chest)
            else
                -- Se o toggle estiver desativado, remove o ESP
                local nameTag = chest:FindFirstChild("NameTag")
                if nameTag then
                    nameTag:Destroy()
                end
            end
...
local function createESP(player)
    ...
    -- Criando o TextLabel para exibir o nome e a distância
    ...
    label.Text = player.Name .. " - Distância: 0"
...
-- Função para atualizar as distâncias
local function updateESP()
    for player, billboard in pairs(ESPs) do
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            billboard.TextLabel.Text = string.format("%s - Distância: %.1f", player.Name, distance)
        end
    end
end
...
local Toggle = Tabs.Players:AddToggle("Esp Player", {Title = "Esp Player", Default = false})
Toggle:OnChanged(function(value)
    Enabled = value
    if Enabled then
        enableESP()
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Try Luck", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Dough King", Default = false })
Toggle:OnChanged(function(Value)
...
local ToggleSpawnCake = Tabs.Sub:AddToggle("ToggleSpawnCake", {Title = "Auto Spawn Cake Prince", Default = false })
...
ToggleChest = Tabs.Sub:AddToggle("ToggleChest", {Title = "Auto Chest", Default = false })
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Stop When Have God's Chalice& Fist Of Darkness", Default = false })
Toggle:OnChanged(function(Value)
...
spawn(function()
    while task.wait(0.5) do
        if getgenv().StopChest then
            local player = game.Players.LocalPlayer
            if not player then
                -- pula para a próxima iteração usando 'else'
            else
                ...
                if backpack and character then
                    if backpack:FindFirstChild("Fist of Darkness") or character:FindFirstChild("Fist of Darkness") or 
                       backpack:FindFirstChild("God's Chalice") or character:FindFirstChild("God's Chalice") then
                        ...
                        break  -- sai do loop porque encontrou o item
                    end
                end
            end
        end
...
local TpDelay = 0.2 -- Tempo entre TPs (em segundos, ajustável)
local LocalPlayer = game:GetService("Players").LocalPlayer
local Enabled = false -- Controle do Toggle
local ChestCounter = 0 -- Contador de baús
...
-- Função para ordenar objetos por distância do jogador
...
local function Teleport(Goal)
    toggleNoclip(true)
    local RootPart = getCharacter().HumanoidRootPart
    RootPart.CFrame = Goal -- Teleporta diretamente para o baú mais próximo
    toggleNoclip(false)
end


-- Implementação do Antikick
spawn(function()
...
-- Reseta o jogador após pegar 3 baús
local function resetCharacter()
    LocalPlayer.Character:BreakJoints() -- Reseta o personagem
    ChestCounter = 0 -- Reseta o contador de baús
end
...
local function main()
    while Enabled do
        ...
        if #Chests > 0 then
            Teleport(Chests[1].CFrame) -- Teleporta instantaneamente para o baú mais próximo
            ChestCounter = ChestCounter + 1
            print("Baú coletado:", ChestCounter)

            -- Reseta o personagem após pegar 3 baús
        ...
        task.wait(TpDelay) -- Espera o tempo configurado antes de ir para o próximo baú
    end
end
...
local Toggle = Tabs.Sub:AddToggle("Teleport to Chests", {Title = "TP Chests (Chances de kick)", Default = false})

Toggle:OnChanged(function(value)
    Enabled = value
    print("Toggle changed:", value)
    if Enabled then
        spawn(main) -- Inicia o script
    else
        print("Script desativado.")
    end
end)

Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Get Cyborg", Description = "Buy Chip And Kill Order", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Get Ghoul", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto UP Observation V2", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Farm Observation", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Touch Pad Haki", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sub:AddToggle("Toggle", {Title = "Auto Buy Legendary Sword", Default = false})
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Quest:AddToggle("Toggle", {Title = "Auto Get Rainbow Haki", Default = false})
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Quest:AddToggle("Toggle", {Title = "Auto Second_Sea", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Quest:AddToggle("Toggle", {Title = "Auto Dark Dagger", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().AutoRipIndra = Value
    StopTween(getgenv().AutoRipIndra)
end)
...
Toggle = Tabs.Quest:AddToggle("Toggle", {Title = "Auto Valkyrie", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().AutoRipIndra = Value
    StopTween(getgenv().AutoRipIndra)
end)
...
local ToggleHallow = Tabs.Quest:AddToggle("ToggleHallow", {Title = "Auto Hallow Scythe [Fully]", Default = false })
...
Toggle = Tabs.Quest:AddToggle("Toggle", {Title = "Auto Get CDK", Description = "Teleport To Claim CDK", Default = false })
Toggle:OnChanged(function(Value)
...
-- Toggle simples
local TweenToFruitToggle = Tabs.Fruit:AddToggle("TweenToFruit", {
...
-- Função de tween com offset
local function TweenToPosition(targetCFrame)
    local Character = Player.Character or Player.CharacterAdded:Wait()
    local HumanoidRootPart = Character:WaitForChild("HumanoidRootPart")
    
    -- Adiciona offset para não colidir com o fruto
    local offset = CFrame.new(0, 3, 0) 
    local tweenInfo = TweenInfo.new(1, Enum.EasingStyle.Linear)
    local tween = TweenService:Create(HumanoidRootPart, tweenInfo, {CFrame = targetCFrame * offset})
    tween:Play()
    tween.Completed:Wait() -- Espera o tween terminar antes de criar outro
end

-- Loop principal
spawn(function()
    while wait(0.5) do  -- aumenta o wait pra reduzir bug
        if TweenToFruitToggle.Value then
            local fruits = {}
            for _, v in pairs(game.Workspace:GetChildren()) do
                if v:IsA("Tool") and v:FindFirstChild("Handle") and string.find(v.Name, "Fruit") then
                    table.insert(fruits, v.Handle)
                end
            end

            -- Tween para cada fruta
            for _, handle in pairs(fruits) do
...
Toggle = Tabs.Fruit:AddToggle("Toggle", {Title = "Teleport To Fruit [ Hop Server ]", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.Fruit:AddToggle("Auto Store", {Title = "Auto Store", Default = false})
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Penetrate Rocks When Boat Runs", Default = true })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", { Title = "Teleport To Migare Island", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", { Title = "Teleport To Highest Point", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", { Title = "Teleport To Advanced Fruit Dealer", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", { Title = "Lock Moon And On Race V3", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", { Title = "Teleport To Blue Gear", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Teleport To Kitsune Island",Default = false })
Toggle:OnChanged(function(value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Summon Soul EmBer",Default = false })
Toggle:OnChanged(function(value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Collect Azure Wisp",Default = false })
Toggle:OnChanged(function(value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Teleport To Frozen Dimension", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Attack Leviathan", Default = false })
Toggle:OnChanged(function(state)
...
Toggle = Tabs.Sea:AddToggle("MyToggle", {Title = "Teleport Prehistoric Island", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Collect Bone", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Sea:AddToggle("Toggle", {Title = "Auto Collect Egg", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Other:AddToggle("Toggle", {Title = "Auto Attack Hydra Mob And Collect Ember", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Other:AddToggle("Toggle", {Title = "Auto Collect FireFlower", Default = false })
Toggle:OnChanged(function(Value)
...
local RunService = game:GetService("RunService")
Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Teleport To The Draco Clan Trial Door", Default = false})
Toggle:OnChanged(function(Value)
...
local Normal = Tabs.Race:AddSection("Race Normal")

Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Auto Upgrade Race V2", Default = false })
Toggle:OnChanged(function(Value)
...
local V4 = Tabs.Race:AddSection("Race V4")
...
Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Teleport Ancient Clock", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Auto Buy Gear", Default = false})
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Auto Finish Train Quest", Default = false})
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Auto Trial Race", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.Race:AddToggle("Toggle", {Title = "Auto Kill Player After Trial V4", Default = false })
Toggle:OnChanged(function(Value)
...
ImageButton.Position = UDim2.new(0.91, 0, 0.07, 0)
...
-- Função para alternar a GUI Fluent
...
-- Clique / toque para alternar GUI
ImageButton.MouseButton1Click:Connect(ToggleFluent)
