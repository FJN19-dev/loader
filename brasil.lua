    ...
    while task.wait(0.1) do
        if _G.Thunder then
            pcall(function()
                ...
                if not boss then return end
                if not boss:FindFirstChild("HumanoidRootPart") 
                or not boss:FindFirstChild("Humanoid") then return end
                if boss.Humanoid.Health <= 0 then return end

                -- Teleporta primeiro para o ponto fixo
                pcall(function()
                    topos(CFrameBoss)
                end)
                task.wait(0.2) -- espera pequena para estabilizar posição
