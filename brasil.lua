...
-- =====================================
local Zones = {
    ["Lv 1"] = CFrame.new(-21998.375, 30, -682.309),
    ["Lv 2"] = CFrame.new(-26779.52, 30, -822.858),
    ["Lv 3"] = CFrame.new(-31171.957, 30, -2256.937),
    ["Lv 4"] = CFrame.new(-34054.687, 30, -2560.12),
    ["Lv 5"] = CFrame.new(-38887.555, 30, -2162.99),
    ["Lv 6"] = CFrame.new(-44541.762, 30, -1244.858),
    ["Lv Infinite"] = CFrame.new(-10000000, 45, 37016.25)
}
...
_G.BoatFly = false
getgenv().SpeedBoat = false
getgenv().AutoPressW = false
getgenv().NoClipShip = false
...
RunService.RenderStepped:Connect(function()
    if not getgenv().SpeedBoat then return end
...
Sea:AddToggle({
    ...
    Default = false,
    Callback = function(v)
        getgenv().NoClipShip = v
    end
})

task.spawn(function()
    while task.wait() do
        if getgenv().NoClipShip then
            for _, boat in pairs(workspace.Boats:GetChildren()) do
...
-- =========================
-- VARIÁVEL GLOBAL (1 TOGGLE)
-- =========================
getgenv().AutoSailFly = false

Sea:AddToggle({
    Name = "Auto Sail Fly (Safe)",
    Default = false,
    Callback = function(v)
        getgenv().AutoSailFly = v
    end
})
...
local function CheckEnemiesBoat()
    ...
    return enemies:FindFirstChild("FishBoat")
        ...
        or enemies:FindFirstChild("MarineBrigade")
        or false
end
...
local function CheckShark()
    ...
    for _, v in pairs(enemies:GetChildren()) do
        ...
        if v.Name == "Shark" and hum and hum.Health > 0 and root then
            if (root.Position - hrp.Position).Magnitude <= 300 then
                return true
            end
        end
    end
...
local function CheckPiranha()
    ...
    for _, v in pairs(enemies:GetChildren()) do
        local hum = v:FindFirstChild("Humanoid")
        local root = v:FindFirstChild("HumanoidRootPart")
        if v.Name == "Piranha" and hum and hum.Health > 0 and root then
            if (root.Position - hrp.Position).Magnitude <= 300 then
                return true
            end
        end
...
local function HasSeaEnemy()
    ...
    return CheckEnemiesBoat()
        or CheckShark()
        or CheckPiranha()
        or enemies:FindFirstChild("Terrorshark")
        or enemies:FindFirstChild("Fish Crew Member")
        or enemies:FindFirstChild("Haunted Crew Member")
        or enemies:FindFirstChild("SeaBeasts")
        or enemies:FindFirstChild("Leviathan")
        or false
end
...
-- =========================
-- AUTO PULO QUANDO TEM INIMIGO
-- =========================
task.spawn(function()
    while task.wait(0.2) do
        if getgenv().AutoSailFly then
            local char = LocalPlayer.Character
            local hum = char and char:FindFirstChild("Humanoid")

            if hum and hum.Sit and HasSeaEnemy() then
...
-- =========================
-- AUTO SAIL FLY (MESMO TOGGLE)
-- =========================
RunService.RenderStepped:Connect(function(dt)
    if not getgenv().AutoSailFly then return end

    local char = LocalPlayer.Character
    local hum = char and char:FindFirstChild("Humanoid")
    if not hum or not hum.Sit then return end

    local targetCF = Zones[_G.DangerSc]
    ...
    for _, boat in pairs(Workspace.Boats:GetChildren()) do
        ...
        if seat and seat.Occupant == hum then
            ...
            if dir.Magnitude > 10 then
                local move = dir.Unit * SetSpeedBoat * dt
                local newPos = currentPos + move

                boat:SetPrimaryPartCFrame(
                    CFrame.new(newPos.X, targetPos.Y, newPos.Z)
                )
            end
        end
    end
