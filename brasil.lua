...
local HopToggle = Tabs.Fruit:AddToggle("HopToggle", {
...
local function TPToFruitHop(targetCFrame)
    local Character = Player.Character or Player.CharacterAdded:Wait()
    local HRP = Character:WaitForChild("HumanoidRootPart")
...
spawn(function()
    while task.wait(0.5) do
        if getgenv().TeleportToFruitHop then

            local foundFruit = false

            for _, v in ipairs(Workspace:GetChildren()) do
                if v:IsA("Model") and v:FindFirstChild("Handle") and v.Name:find("Fruit") then
                    
                    TPToFruitHop(v.Handle.CFrame)
                    foundFruit = true
                    break
                end
            end
