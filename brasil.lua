...
_G.PlayersList = GetPlayerList()[1]

local PlayerDropdown = PlayersTab:AddDropdown({
    Name = "Choose Player",
    Options = GetPlayerList(),
    Default = _G.PlayersList,
    Callback = function(Value)
    ...
    Callback = function(Value)
        ...
        task.spawn(function()
            while _G.TpPly do
                task.wait()
                pcall(function()
                    local target = Players[_G.PlayersList]
...
local Section = PlayersTab:AddSection({"Visual"})
...
Toggle1:Callback(function(Value)
    ...
    local ESP_COLOR = Color3.fromRGB(170, 0, 255)
    local connections = {}

    local function removeESP(player)
        if player.Character and player.Character:FindFirstChild("Head") then
            local gui = player.Character.Head:FindFirstChild("PlayerESP")
    ...
    local function createESP(player)
        ...
        local function onCharacter(char)
            ...
            if not head or not Value then return end
            ...
            img.Position = UDim2.new(0, 0, 0, 4)
            ...
            connections[player] = RunService.RenderStepped:Connect(function()
                if not Value or not char.Parent then
                    removeESP(player)
                    return
                end
...
local ToggleFruit = Fruit:AddToggle({
    Name = "Finder Fruits",
    Description = "",
    Default = false
})

ToggleFruit:Callback(function(Value)
    getgenv().AutoFruitFarm = Value
end)
...
task.spawn(function()
    while task.wait(0.2) do
        if not getgenv().AutoFruitFarm then continue end
        if not Player.Character then continue end

        local foundFruit = false

        -- ===== TP PARA FRUTAS NO MAPA =====
        for _, obj in pairs(workspace:GetChildren()) do
